...
int main(int argc, char** argv)
{
	...
	//cross dir and left to get basis2

	glm::vec3 gaze = dir;//make sure the dir remain the same
	glm::vec3 base1 = glm::normalize(glm::cross(gaze, up));
	glm::vec3 base2 = glm::cross(gaze, base1);
	float f = sqrt((lookAt - eye)[0] * (lookAt - eye)[0] + (lookAt - eye)[1] * (lookAt - eye)[1] + (lookAt - eye)[2] * (lookAt - eye)[2]);
	float height = 2 * f * tan(fovy / 2);
		...
		if (raytracing)
		{
			...
			for (int y = 0; y < SCR_HEIGHT; y++)
			{
				...
				for (int x = 0; x < SCR_WIDTH; x++)
				{
					...
					//Build the view rectangular
					float x_m = x - (float)SCR_WIDTH / 2 + 0.5f;
					float y_m = y - (float)SCR_HEIGHT / 2 + 0.5f;
					float height_f = height / SCR_HEIGHT;
					float width_f = width / SCR_WIDTH;
					glm::vec3 currentDir = (lookAt - eye) + x_m * width_f * base1 + y_m * height_f * base2;
